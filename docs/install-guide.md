...
2. 최신 릴리스에서 아래 3개 파일 다운로드:
   ...
   - `manifest.json`
   - `styles.css`
3. Obsidian vault 폴더에서 `.obsidian/plugins/clipboard-to-slack/` 디렉토리 생성
4. 다운로드한 3개 파일을 해당 디렉토리에 복사

```bash
# 예시 (macOS/Linux)
mkdir -p <vault경로>/.obsidian/plugins/clipboard-to-slack
cp main.js manifest.json styles.css <vault경로>/.obsidian/plugins/clipboard-to-slack/
```
...
4. **설치된 플러그인** 목록에서 **Clipboard to Slack** 토글 켜기
5. Obsidian 재시작 (필요 시)
...
- **플러그인이 보이지 않는 경우**: `.obsidian/plugins/clipboard-to-slack/` 경로에 `main.js`, `manifest.json` 파일이 있는지 확인
